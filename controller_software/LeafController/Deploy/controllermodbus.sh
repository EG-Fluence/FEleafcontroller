...
localhelp () {
  echo "Usage:"
  echo "  ${SCRIPTNAME}"
  echo "    Get controller modbus status on local computer"
  echo "  ${SCRIPTNAME} remote_IP rootpwd"
  echo "  ${SCRIPTNAME} file_with_remote_IPs rootpwd"
  echo "    Get controller modbus status  on remote computer"
...
getModbusStatus () {
  ...
  systemctl status fluence-modbus | egrep -i "Active|Loaded|Main PID"
  echo "Used ports:"
  netstat -nap | grep 502
  #popd > /dev/null 2>&1
  #exit 0
}
